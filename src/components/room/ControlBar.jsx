  ...
  return (
    <div className="qz-control-bar">
      ...
      <div className="qz-control-bar__group qz-control-bar__group--center">
        ...
        <button className={`qz-ctrl-btn ${participantsOpen ? 'qz-ctrl-btn--active' : ''}`} onClick={onToggleParticipants} aria-pressed={participantsOpen} aria-label="Toggle participants list" title="View participants">
          <Users size={19} strokeWidth={2.2} />
          <span className="qz-ctrl-btn__count">{participantCount}</span>
          <span>People</span>
        </button>
        <button className="qz-ctrl-btn" onClick={onMore} aria-label="More options" title="More options">
      ...
      <div className="qz-control-bar__group qz-control-bar__group--right">
        <button className="qz-ctrl-leave" onClick={canEndMeeting ? onEndMeeting : onLeave}>
          <PhoneOff size={17} strokeWidth={2.3} /> <span>{canEndMeeting ? 'End meeting' : 'Leave'}</span>
        </button>
      </div>
    </div>
  );

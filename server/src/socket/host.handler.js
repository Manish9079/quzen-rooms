...
export function registerHostHandlers(io, socket) {
  socket.on('host:removeParticipant', async ({ userId } = {}, ack) => {
    try {
      ...
      for (const sid of presenceStore.socketsInRoom(socket.data.roomCode)) {
        const meta = presenceStore.getMeta(sid);
        if (meta?.userId === userId) {
          io.to(sid).emit('host:removedYou');
          io.sockets.sockets.get(sid)?.leave(socket.data.roomCode);
          presenceStore.removeSocket(sid);
        }
      }
      io.to(socket.data.roomCode).emit('presence:userLeft', { userId, reason: 'removed' });
      io.to(socket.data.roomCode).emit('presence:participantCount', { count: presenceStore.roomSize(socket.data.roomCode) });
      ack?.({ ok: true });
    } catch (err) {
  ...
  socket.on('host:setCoHost', async ({ userId } = {}, ack) => {
    try {
      ...
      io.to(socket.data.roomCode).emit('host:roleChanged', { participant });
      ack?.({ ok: true });
    } catch (err) {
      ack?.({ ok: false, message: err.message });
    }
  ...
  socket.on('host:lockRoom', async (_payload, ack) => {
    try {
      ...
      io.to(socket.data.roomCode).emit('host:roomLocked', { isLocked: room.isLocked, by: participant.userId });
      ack?.({ ok: true });
    } catch (err) {
      ack?.({ ok: false, message: err.message });
    }
  ...
  socket.on('host:unlockRoom', async (_payload, ack) => {
    try {
      ...
      io.to(socket.data.roomCode).emit('host:roomLocked', { isLocked: room.isLocked });
      ack?.({ ok: true });
    } catch (err) {
      ack?.({ ok: false, message: err.message });
    }
  ...
  socket.on('host:approveWaiting', async ({ userId } = {}, ack) => {
    try {
      await requireRole(socket, ['HOST', 'CO_HOST']);
      const waiting = presenceStore.listWaitingRoom(socket.data.roomCode).find((w) => w.userId === userId);
      if (!waiting) throw new Error('That user is no longer waiting.');

      const waitingSocket = io.sockets.sockets.get(waiting.socketId);
      presenceStore.removeFromWaitingRoom(socket.data.roomCode, userId);
      if (waitingSocket) {
        waitingSocket.data.waitingRoomId = null;
        await completeJoin(io, waitingSocket, socket.data.roomCode, undefined, { bypassLock: true });
      }
      io.to(socket.data.roomCode).emit('host:waitingRoomUpdate', { waiting: presenceStore.listWaitingRoom(socket.data.roomCode) });
      ack?.({ ok: true });
    } catch (err) {
      ack?.({ ok: false, message: err.message });
    }
